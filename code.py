...
def score():
    ...
    highscore=0
    with open("highscore.txt", "r") as obj: #check highscore
        for line in obj.readline():
            if score >= int(line):
                ...
                highscore = line
    obj.close()
    with open("highscore.txt", "w") as obj2: #update highscore to txt file
        obj2.write(str(highscore))
    obj2.close()
    
    hs = "Your highscore is " + str(highscore) #display highscore
    h= Label(f1,
             text=hs,
             width=90,
             pady=10,
...
def initiate(): #generate question, option, answer list; make useranswer list, set qno=0
    ...
    ques_nos = tuple(random.sample(range(1, 43), 10))
